...
{     
    ...
    //int *p_table = (int *)p_dir;

    //fillMemory((char *)p_dir, 0, PAGE_SIZE);
    //fillMemory((char *)p_table, 0, PAGE_SIZE);
    ...
    *p_dir = (PAGE_TABLE_BASE + (MAX_PGTABLES_SIZE * (pid - 1))) | PG_USER_PRESENT_RW;
    ...
    // Get pointers to page directory and page table
    int *p_dir = (int *)p_dir_addr;
    int *p_table = (int *)(p_dir_addr + PAGE_SIZE); // Page table address
    ...
    p_table[KEYBOARD_BUFFER / PAGE_SIZE] = KEYBOARD_BUFFER | PG_USER_PRESENT_RW;

    // 3. USER_PID_INFO should be PG_USER_PRESENT_RO
    p_table[USER_PID_INFO / PAGE_SIZE] = USER_PID_INFO | PG_USER_PRESENT_RO;

    // 4. GDT_LOC should be PG_USER_PRESENT_RO
    p_table[GDT_LOC / PAGE_SIZE] = GDT_LOC | PG_USER_PRESENT_RO;

    // 5. VIDEO_RAM should be PG_USER_PRESENT_RW
    p_table[(int)VIDEO_RAM / PAGE_SIZE] = (int)VIDEO_RAM | PG_USER_PRESENT_RW;

    // 6. All of kernel space should be PG_KERNEL_PRESENT_RW
    for (int i = KERNEL_BASE; i < KERNEL_LIMIT; i += PAGE_SIZE)
    {
        p_table[i / PAGE_SIZE] = i | PG_KERNEL_PRESENT_RW;
    }

    // Set up the first entry in the page directory to point to the page table
    // 7. The first (and only entry) in the page directory should be PG_USER_PRESENT_RW
    p_dir[0] = (int)(p_dir_addr + PAGE_SIZE) | PG_USER_PRESENT_RW;//*/
} 
...
int requestSpecificPage(int pid, char *pageMemoryLocation, char perms)
{
    ...
    // *******If page table start location is not at zero
    // Using more of the constants, make sure your getting the right numbers
    if (p_frame_num != PAGEFRAME_AVAILABLE)
    {
        ...
        //    Insert the page frame into the process's page table
